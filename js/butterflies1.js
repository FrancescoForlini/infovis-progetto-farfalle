...
function creaBordoSVG(svg, height, width){ //bordi che delimitano l'area delle farfalle 
...
function moveInCoordinates(data, i){
...
function makeButterfly(g_butterflies){
...
function changePositionButterflies(dat, j, butterflies){
...
d3.json("data/dataset.json").then( function(data){
	...
	var butterflies = svgg.selectAll('.ng')
					.data(data)
					.enter()
					.append("g")
					.attr("class", "ng");

	makeButterfly2(butterflies);
	...
	d3.selectAll(".ng").on("click", function() {
    	data = changePositionButterflies(data, i, butterflies);
	});
